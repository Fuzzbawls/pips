...
my $bipnum = 0;
while (++$bipnum <= $topbip) {
	my $fn = sprintf "pip-%04d.mediawiki", $bipnum;
	-e $fn || next;
	open my $F, "<$fn";
	...
	while (<$F>) {
		...
		if ($field eq 'PIP') {
			die "$fn claims to be PIP $val" if $val ne $bipnum;
			...
			$authoremail =~ s/(?<=\D)$bipnum(?=\D)/<PIPNUM>/g;
			...
			if (not $found{'Comments-URI'}) {
				my $first_comments_uri = sprintf('https://github.com/pivx-project/pips/wiki/Comments:PIP-%04d', $bipnum);
